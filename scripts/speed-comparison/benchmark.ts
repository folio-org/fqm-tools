...
> {
  ...
  Object.entries(queries).forEach(([baseLabel, { entityType, queries, fields }]) => {
    const entityTypeId = getThisVersionOf(entityType);

    queries.forEach((queryList) =>
      fields.slice(0, includeEachFieldsVariant ? fields.length : 1).forEach((fieldset) => {
...
async function measure<T, R>(
  ...
  const initialLength = measures[name].length;

  while (measures[name].length < 5 + initialLength || Date.now() - overallStart < maxTimeMs) {
    ...
    process.stdout.write(
      `${name} (${description}): ${measures[name].length - initialLength} samples, last ${end - start}ms`,
    );
  }
  ...
  process.stdout.cursorTo(0);
  console.log(
    `${name} (${description}): approx ${(measures[name].reduce((a, b) => a + b, 0) / measures[name].length).toFixed(
      2,
    )}ms (${measures[name].length} samples)`,
  );
...
async function measureEntityTypes() {
  const entityTypes = await ky.get('entity-types').json<{ id: string; label: string }[]>();

  await measure(
    'get-all-entity-types',
    {
      fn: () => ky.get('entity-types'),
      describe: async (response) => `${(await response.json<never[]>()).length} entities`,
    },
    5000,
  );
      ...
      const checkIfDone = async () => {
        try {
          const response = await (
            await ky.get(`lists/${listId}`)
          ).json<{
            successRefresh?: { status?: string; recordsCount: number };
            inProgressRefresh?: { status?: string };
            failedRefresh?: { status?: string };
          }>();
          if (response.successRefresh?.status === 'SUCCESS' && !('inProgressRefresh' in response)) {
      ...
      label + '|all',
      {
          ...
          const list = await (
            await ky.post('lists', {
              json: {
                ...
                description: JSON.stringify({ label, entityTypeId, query, fields }, null, 2),
                entityTypeId,
        ...
        teardown: async (list) => {
          ...
          for (const {
            ...
            measures[label + '|query'].push(toSeconds(parse(WAIT_FOR_QUERY_COMPLETION)) * 1000);

            measures[label + '|import-results'] = measures[label + '|import-results'] ?? [];
            measures[label + '|import-results'].push(toSeconds(parse(IMPORT_RESULTS)) * 1000);
          }
