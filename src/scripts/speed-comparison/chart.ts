...
import { CategoryScale, Chart, ChartConfiguration, LinearScale } from 'chart.js';
...
keys
  .map((f) => [f, readFileSync(`raw-results/${f}-descriptions.json`, 'utf8')] as const)
  .map(([f, s]) => [f, JSON.parse(s) as Record<string, string>] as const)
  .forEach(([f, r]) => (descriptions[f] = r));

const metrics = [...new Set<string>([...Object.values(results).flatMap((o) => Object.keys(o))])].toSorted((a, b) =>
  a.localeCompare(b),
);
  ...
  <table>
    ...
    <tbody>
      ${metrics
        .map(
          (m, metricIndex) => `
        <tr style="${metricIndex % 2 === 0 ? '' : 'background-color: #eee;'}" data-metric="${m}">
          <th scope="row" rowspan="${keys.length}"><a href="#${m}">${m}</a><br />${[
            ...new Set(Object.values(descriptions).map((v) => v[getDescriptionMetricKey(m)])),
          ].join('<br />')}</th>
          ${keys
            .map((k, keyIndex) => {
              if (!results[k][m]) {
              ...
              const stats = new Stats().push(...results[k][m].map((x) => x / 1000));
              const prevStats =
                keyIndex === 0 || !results[keys[keyIndex - 1]][m]
                  ? null
                  : new Stats().push(...results[keys[keyIndex - 1]][m].map((x) => x / 1000));
              ...
              return `
                ...
                ${prevStats ? purdyPercent((stats.percentile(50) / prevStats.percentile(50) - 1) * 100) : '<td>-</td>'}
              `;
            })
            .join(`</tr><tr style="${metricIndex % 2 === 0 ? '' : 'background-color: #eee;'}" data-metric="${m}">`)}
        </tr>`,
        )
        .join('')}
    </tbody>
      ...
      ${keys
        ...
        <tr>
          ...
          ${
            prevStats
              ? `${purdyPercent(
                  ((stats.min.sum - prevStats.min.sum) / prevStats.min.sum) * 100,
                  stats.min.sum.toFixed(3) + '<br/>',
                  true,
                )}${purdyPercent(
                  ((stats.median.sum - prevStats.median.sum) / prevStats.median.sum) * 100,
                  stats.median.sum.toFixed(3) + '<br/>',
                )}${purdyPercent(
                  ((stats.max.sum - prevStats.max.sum) / prevStats.max.sum) * 100,
                  stats.max.sum.toFixed(3) + '<br/>',
                  true,
                )}${purdyPercent(
                  ((stats.mean.sum - prevStats.mean.sum) / prevStats.mean.sum) * 100,
                  stats.mean.sum.toFixed(3) + '<br/>',
                )}${purdyPercent(
                  ((stats.stddev.sum - prevStats.stddev.sum) / prevStats.stddev.sum) * 100,
    ...
    <table>
      ...
      <tbody>
        ${keys
          ...
          .map((k) => {
            const stats = new Stats().push(results[k][metric].map((x) => x / 1000));
            return `
              <tr>
                <th scope="row" style="font-family: monospace">${k}</th>
                ...
                <td class="nerd">${stats.max!.toFixed(3)}</td>
                <td class="nerd">${Math.abs(stats.range()[1] - stats.range()[0]).toFixed(3)}</td>
...
function purdyPercent(percent: number, src: string = '', isNerd: boolean = false) {
  const val = percent >= 0 ? `${src}+${percent.toFixed(2)}%` : `${src}${percent.toFixed(2)}%`;
  const clazz = isNerd ? 'nerd' : '';
  const template = (color: string) => `<td style="background-color: ${color}" class="${clazz}">${val}</td>`;
