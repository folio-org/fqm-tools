...
| `x-fqm-visibility`          | `all`, `query-only`, `results-only`, `hidden`, or `hidden-non-queryable` | `all`, or `results-only` for nested objects inside arrays | Controls if this field is usable everywhere (`all`), only in queries (`query-only`), only in results (`results-only`), completely hidden from the UI (`hidden`), or hidden and non-queryable (`hidden-non-queryable`). |
...
```json5
{
  'x-fqm-source': {
    ...
    columnName: 'location_name', // the internal name of the field in that entity type
  },
}
```
